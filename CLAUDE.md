...
### ⚠️ 「実績」evidence の書き方（2026.09.22追記）

2026年9月の全件検証で、「実績」evidence 101件を削除した。削除した記載には
はっきりした共通パターンがあり、いずれもCIで検出するようにしている。

**1. 出典名を創作しない**

削除した記載の多くは、以下のような名称を出典に掲げていた。**いずれも検索しても
実在しない。** 官公庁の刊行物らしい名前を組み立てただけのものである。

```
✗ 国土交通省 政策実績アーカイブ      ✗ デジタル庁政策成果報告書
✗ 内閣府政策実績資料                 ✗ 復興庁 政策実績白書
✗ 環境省政策発表・閣僚活動記録       ✗ 法務省職務報告資料
```

実在する資料名とURLを書くこと。閣僚・政務三役の在任時期であれば、
首相官邸の内閣別名簿（`kantei.go.jp`）、内閣府大臣ページ（`cao.go.jp/minister/`）、
衆参の議員プロフィールが一次情報として使える。

**2. 資料体の名前だけでは出典にならない**

`src:"参議院会議録"` とだけ書かれた40件を削除した。どの会期の、どの委員会の、
いつの会議録なのかが特定できなければ読者は検証できない。会議録を引くなら
国会会議録検索システム（https://kokkai.ndl.go.jp/）の該当ページのURLを付ける。
...
これらは検証可能な命題を含んでいないため、正しいとも誤りとも言えない。
**法案名・予算額・日付・閣議決定名のいずれかがなければ書かない。**

**4. 役職と在任時期は必ず照合する**

役職名だけ見ると妥当に見えても、時期を照合すると破綻している例が多かった。

- 記載された役職に**就任した事実がない**（16件）
- 役職は実在するが**在任時期が記載と一致しない**（4件）— 記載の日付時点では
  別の役職に就いていた、あるいはまだ当選していなかった
- 記載された政策が**その役職の所管外**（19件）
- 在任**後**の出来事を実績としている — ラピダス設立は梶山弘志の退任10か月後、
  3か年緊急対策の完了は石井啓一の退任1年半後

**4-2. 検出パターンは末尾一致で書かない（2026.09.22追記）**

`/政策実績白書$/` のように末尾一致で書いていたため、「復興庁 政策実績白書資料」の
ように語尾を1語足しただけの出典がCIを素通りしていた。**部分一致で書くこと。**

首長20件が使っていた「◯◯県庁実績」「◯◯県庁政策実績」も同じ構造である。
県庁が「実績」という名の刊行物を出しているわけではない。出典に自治体名を書くなら、
その自治体が実際に公開している計画名・報道発表・会見録のURLを指定する。

**4-3. evidence を持たない議員に評価点を付けない（2026.09.22追記）**

裏付けとなる evidence を1件も持たない議員には、総合スコア・ランク・8軸評価を
付けない。`total:null, rank:"", axes:null, survey:"未評価"` とし、サイト上は
「評価なし（裏付け資料の確認中）」と表示する。

2026年9月時点で、評価点だけがあって evidence を持たない議員は445人（全781人の
57%）いた。**評価点は8軸の合計から機械的に算出されるため、軸に裏付けが無ければ
評価点にも裏付けが無い。** 軸だけ残して総合点を消す、という中途半端な状態を作らない。
...
知事時代の業績を主語をぼかして国会議員の実績として書くと、読者を誤認させる。
主体と時期を明示するか、国政での役職に基づく記述に差し替えること。

---

### data_core.js の同期ルール（2026.09.22追記）

**議員一覧ページ（politicians.html）は `data.js` ではなく `data_core.js` を読む。**
`data.js` だけ直しても一覧ページには反映されない。2026年9月の evidence 101件削除では
これを忘れ、一覧の「根拠」件数が830件（実際は552件）のまま表示されていた。

`data.js` を更新したら、必ず `data_core.js` と `politicians_base.json` にも
同じ変更を反映すること。`scripts/validate_core_sync.js` がCIで両者の一致を検証する。
